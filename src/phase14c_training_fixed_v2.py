...
Phase 14c: Training - Fixed V2 (Anomaly Features + Metric Names)
...
# ============================================================
# LOAD DATA WITH RAW ANOMALY FEATURES
# ============================================================
def load_data_with_raw_features():
    """بارگذاری داده با ویژگی‌های خام Anomaly"""
    log("=" * 70)
    log("LOADING DATA WITH RAW ANOMALY FEATURES")
    log("=" * 70)

    # بارگذاری داده خام
    df_raw = pd.read_csv(RAW_DATA)
    log(f"  Raw data: {df_raw.shape}")
    ...
    available = [c for c in anomaly_features_raw if c in df_raw.columns]
    log(f"  Available raw anomaly features: {available}")

    # بررسی مقادیر منفی
    log(f"\n  Negative values in raw data:")
    for col in available:
        neg_count = (df_raw[col] < 0).sum()
        log(f"    {col}: {neg_count} negative")
    ...
    # حذف ستون‌های تکراری از train_df و val_df (اگر وجود دارند)
    cols_to_drop = [c for c in available if c in train_df.columns]
    if cols_to_drop:
        log(f"\n  Dropping existing anomaly features from train/val: {cols_to_drop}")
        train_df = train_df.drop(columns=cols_to_drop)
        val_df = val_df.drop(columns=cols_to_drop)

    # ادغام ویژگی‌های خام با نام‌گذاری جدید
    raw_features = df_raw[["Timestamp"] + available].copy()

    # تغییر نام ستون‌ها به _raw
    rename_dict = {c: f"{c}_raw" for c in available}
    raw_features = raw_features.rename(columns=rename_dict)
    ...
    # لیست ویژگی‌های خام جدید
    raw_features_new = [f"{c}_raw" for c in available]

    # بررسی Missing بعد از merge
    log(f"\n  Missing values after merge:")
    for col in raw_features_new:
        log(f"    Train {col}: {train_df[col].isnull().sum()}")
        log(f"    Val {col}: {val_df[col].isnull().sum()}")

    return train_df, val_df, feature_df, raw_features_new

# ============================================================
# GET METRIC NAMES
# ============================================================
def get_metric_names(history):
    """استخراج نام دقیق متریک‌ها از history"""
    ...
    for k in keys:
        if k.startswith("val_precision") and val_precision_key is None:
            val_precision_key = k
        elif k.startswith("val_recall") and val_recall_key is None:
            val_recall_key = k
        elif k.startswith("precision") and not k.startswith("val") and precision_key is None:
            precision_key = k
        elif k.startswith("recall") and not k.startswith("val") and recall_key is None:
            recall_key = k

    return {
        "val_precision": val_precision_key,
        "val_recall": val_recall_key,
...
def test_class_weights(X_train, y_train, X_val, y_val, params):
    ...
    for name, cw in class_weights_to_test.items():
        ...
        try:
            ...
            # استخراج نام متریک‌ها
            metric_names = get_metric_names(history)

            best_epoch = np.argmin(history.history["val_loss"])
            best_val_loss = history.history["val_loss"][best_epoch]
            best_val_acc = history.history["val_accuracy"][best_epoch]

            # استخراج Precision و Recall با نام‌های صحیح
            if metric_names["val_precision"]:
                best_val_precision = history.history[metric_names["val_precision"]][best_epoch]
                best_val_recall = history.history[metric_names["val_recall"]][best_epoch]
            else:
...
def visualize_results(results_df, histories):
    ...
    if results_df.empty:
        log("  [WARNING] No results to visualize")
        return
    ...
    width = 0.25
    axes[0, 0].bar(x - width, results_df["Best_Val_Precision"], width, label="Precision", color="steelblue")
    axes[0, 0].bar(x, results_df["Best_Val_Recall"], width, label="Recall", color="coral")
    ...
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "14c_class_weight_comparison.png", dpi=100, bbox_inches="tight")
    plt.close()
    log(f"  [OK] Saved: 14c_class_weight_comparison.png")
...
def main():
    log("=" * 70)
    log("PHASE 14c: TRAINING - FIXED V2")
    log("=" * 70)
    ...
    # بارگذاری داده با ویژگی‌های خام
    train_df, val_df, feature_df, raw_features = load_data_with_raw_features()

    # ویژگی‌ها: اصلی + خام Anomaly
    features_main = [c for c in feature_df["Feature"].tolist()
                     if c in train_df.columns and c not in LEAKY_CLF]

    features = features_main + raw_features

    log(f"\n  Total Features: {len(features)}")
    log(f"    Main features: {len(features_main)}")
    log(f"    Raw anomaly features: {len(raw_features)}")
    log(f"    Raw features list: {raw_features}")
    ...
    results_df.to_csv(TABLES_DIR / "14c_class_weight_results.csv", index=False)
    log(f"\n  [OK] Saved: 14c_class_weight_results.csv")
    ...
    # ============================================================
    if results_df.empty:
        log("\n  [ERROR] No results to select best model from")
        return
    ...
    final_callbacks = [
        ...
        ModelCheckpoint(
            filepath=str(MODELS_DIR / "best_anomaly_model_v2.keras"),
            monitor="val_loss",
            save_best_only=True,
            verbose=0,
        ),
    ...
    # استخراج نام متریک‌ها
    metric_names = get_metric_names(final_history)
    ...
    if metric_names["val_precision"]:
        best_val_precision = final_history.history[metric_names["val_precision"]][best_epoch]
        best_val_recall = final_history.history[metric_names["val_recall"]][best_epoch]
    else:
        best_val_precision = 0.0
        best_val_recall = 0.0
    ...
    # ذخیره مدل
    final_model.save(MODELS_DIR / "anomaly_mlp_final_v3.keras")
    log(f"  [OK] Saved: anomaly_mlp_final_v3.keras")
    ...
    summary = {
        ...
        "Raw_Anomaly_Features": ", ".join(raw_features),
    }

    pd.DataFrame(list(summary.items()), columns=["Parameter", "Value"]).to_csv(
        TABLES_DIR / "14c_final_training_summary.csv", index=False
    )
    log(f"  [OK] Saved: 14c_final_training_summary.csv")

    log("\n" + "=" * 70)
    log("PHASE 14c COMPLETE!")
    log("=" * 70)

    LOG_FILE = BASE_DIR / "reports" / "phase14c_log.txt"
